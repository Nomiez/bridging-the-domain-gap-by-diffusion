...
class GenerateConfig:
    checkpoint_dir: str
    model_config: str = "models/cldm_seg_ade20k_multi_step_D.yaml"
    output_dir: str = "image_output/ade20k_generated_domain_ALDM"
    seed: int = 1234
...
def run(config: GenerateConfig):
    ...
    # New: use filtered images
    cur_dir = os.path.dirname(__file__)
    csv_dir = os.path.join(cur_dir, 'ade_val_filtered_outdoor.csv')
    df_temp = pd.read_csv(csv_dir)
    use_image_list = df_temp['name'].to_list()
    domain_prompt_list = ['night scene', 'snowy scene', 'foggy scene', 'rainy scene']
    num_runs = len(domain_prompt_list)
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(dataloader)):
            ...
            base_name = Path(data['label_pth'][0]).stem
            if base_name not in use_image_list:
                continue
            ...
            for j in range(num_runs):
                cur_domain = domain_prompt_list[j]
                cond = {
                    "c_concat": [control],
                    "c_crossattn": [model.get_learned_conditioning([prompt[0] + ', ' + cur_domain])]
                }
                ...
                for k in range(num_img_per_batch):
                    ...
                    domain_name_dict[img_name] = cur_domain
