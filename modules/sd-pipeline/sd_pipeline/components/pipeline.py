...
class Pipeline:
    ...
    def split(
        self, *pipelines: Pipeline, split_array: list[float] | list[int], type: str | None = "abs"
    ) -> Pipeline:
        ...
        def store_output_para(
            ...
            for pipeline in pipelines:
                ...
                if type == "abs" and sum(split_array) > len(input_data):
                    raise RuntimeError(
                        "Absolute number of splitts cannot be greater then the number of data"
                    )
                ...
                for pipeline, share in zip(pipelines, split_array):
                    ...
                    if type == "abs":
                        res += (
                            Pipeline(config, pipeline.functions.copy())
                            ._inject_image_data(input_data[processed : (processed + share)])
                            .run(),
                        )
                        processed += share
                    else:
                        res += (
                            Pipeline(config, pipeline.functions.copy())
                            ._inject_image_data(
                                input_data[processed : [processed + int(share * input_data)]]
                            )
                            .run(),
                        )
