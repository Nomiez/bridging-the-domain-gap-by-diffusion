...
if __name__ == "__main__":
    ...
    lbfs_config = LBFSConfig(input_dir=os.path.join(os.getcwd(), "data/bbox"))
    ...
    result = (
        ...
        .parallel(
            ...
            .for_each(
                SubPipeline.init().step(
                    Rename(config=RenameConfig(rename_function=lambda x: x + "_bbox"))
                )
            ),
        )
        ...
        .split(
            SubPipeline.init().step(
                C2C(
                    config=C2CConfig(
                        output_dir_json="data/output/dataset/annotations/train_labels.json"
                    )
                )
            ),
            SubPipeline.init().step(
                C2C(
                    config=C2CConfig(
                        output_dir_json="data/output/dataset/annotations/val_labels.json"
                    )
                )
            ),
            SubPipeline.init().step(
                C2C(
                    config=C2CConfig(
                        output_dir_json="data/output/dataset/annotations/test_labels.json"
                    )
                )
            ),
